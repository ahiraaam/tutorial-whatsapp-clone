...
export const resolvers: Resolvers = {
  ...
  Message: {
    ...
    async chat(message:any, args, { db }) {
    ...
    async sender(message:any, args, { db }) {
      const { rows } = await db.query(sql`
    ...
    async recipient(message:any, args, { db }) {
      const { rows } = await db.query(sql`
    ...
    isMine(message:any, args, { currentUser }) {
      return message.sender_user_id === currentUser.id;
    },
  },

  Chat: {
    async name(chat:any, args, { currentUser, db }) {
    ...
    async lastMessage(chat:any, args, { db }) {
      const { rows } = await db.query(sql`
    ...
    async participants(chat:any, args, { db }) {
      const { rows } = await db.query(sql`
        SELECT users.* FROM users, chats_users
