...
const resolvers: Resolvers = {
  Message: {
    ...
    async isMine(message:any, args, { injector }) {
  ...
  Chat: {
    async name(chat:any, args, { injector }) {
      const currentUser = await injector.get(Auth).currentUser();
    ...
    async picture(chat:any, args, { injector }) {
      const currentUser = await injector.get(Auth).currentUser();
      if (!currentUser) return null;
    ...
    async messages(chat:any, args, { injector }) {
      return injector.get(Chats).findMessagesByChat(chat.id);
    },

    async lastMessage(chat:any, args, { injector }) {
      return injector.get(Chats).lastMessage(chat.id);
    },

    async participants(chat:any, args, { injector }) {
